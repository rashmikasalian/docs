...
## 🎯 Purpose
The Demand Generation module initiates the recruitment supply chain of Recruit Edge in Mantrika.ai by enabling recruiters to craft structured, data-rich, and market-aware job demands. It leverages both Location Intelligence and Competitive Intelligence to equip recruiters with actionable market insights at the very start of the hiring process. By combining an AI-driven Location Favorability Index (LFI) with a real-time Competitive Market Fit Index (DMFI), this capability ensures that every job demand is not only structured with clear role requirements but also optimized for:
- Where to source candidates—prioritizing cities and regions with the best talent supply, cost efficiency, and hiring success rates.
- How to position the role—benchmarking compensation, skill scarcity, and competitor hiring activity to maximize attractiveness.

## 🗺️ Scope 
To deliver these insights, Demand Generation ingests and processes a broad range of data, including:
...
## 🛠️ Key Capabilities
 🔹 AI-Driven Heatmaps  
Spot high-density talent clusters by skill, city, or region with real-time visual insights to guide sourcing strategy.

🔹 Integration with Workforce Planning  
Align headcount forecasts with actual market availability to reduce sourcing inefficiencies and hiring delays.

🔹 Competitive Insights in Real Time  
Monitor competitor hiring trends, salary benchmarks, and talent movement to stay ahead in high-demand skill areas.

🔹 Skill-Based Demand-Supply Analytics  
Access deep, skill-level market signals to plan demand intelligently, ensuring timing and location match talent supply.
...
<CardGroup cols={2}>
  ...
  >
    >Identifies the best hiring locations using AI-driven insights on talent supply, cost, and success rates.
    >
  ...
  >
    >Real-time benchmarking of role attractiveness against external market data.
    >
    ><span style={{ color: '#007AFF' }}> Learn More →</span>
  </Card>
</CardGroup>
